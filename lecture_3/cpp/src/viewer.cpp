...
void PCLViewer::DisplayPointCloud(std::vector<GMM::Cluster> clusters) {
    ...
    for (unsigned int i = 0; i < clusters.size(); ++i) {
        ...
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> single_color(point_cloud_ptr, color_r, color_g, color_b);
        viewer.addPointCloud<pcl::PointXYZ>(point_cloud_ptr, single_color, "point cloud " + std::to_string(i));
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, "point cloud " + std::to_string(i));
    }
...
//        center_points_ptr->push_back(center_point);
        ...
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> single_color(source_points_ptr, color_r, color_g, color_b);
